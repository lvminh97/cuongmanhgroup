<?php 
	...
	$connect->build_query([
		...
		"gia_tri_cot" => "'{$name_post}','{$link_avt}','{$date}','{$description}','{$description_n}','0'"
	])->insert();
    ...
    $get_id_post = $connect->build_query([
        "table" => "post",
        "select" => "`id_post`",
        "where" => "`name_post` = '$name_post' AND `category` = '0' AND `date` = '{$date}' AND `description_n`='{$description_n}'"
    ])->select();
	...
	header('location: ../admin/dashboard-post.php');
?>