<?php 
	...
	if ($_FILES['avatar']['type'] == 'image/jpeg' || $_FILES['avatar']['type'] == 'image/png' || $_FILES['avatar']['type'] == 'image/jpg') {
        $file_name = utf8tourl($name_post)."-".date('YmdHis', time());
	...
	$connect->build_query([
		"table" => "post",
		"ten_cot" => "`name_post`, `avatar`, `date`, `description`,`description_n`,`category`",
		"gia_tri_cot" => "'{$name_post}','{$link_avt}','{$date}','{$description}','{$description_n}','3'"
	])->insert();
    // lấy id sản phẩm
    $get_id_post = $connect->build_query([
        "table" => "post",
        "select" => "`id_post`",
        "where" => "`name_post` = '$name_post' AND `category` = '3' AND `date` = '{$date}' AND `description_n`='{$description_n}'"
    ])->select();
